...
import static de.hhn.it.vs.distribution.qna.provider.wnck.sockets.QnAServiceServeOneClient.*;
...
public class BDQnAServiceViaSockets implements BDQnAService {
  ...
  public long createArea(final Token userToken, final Area area) throws ServiceNotAvailableException, IllegalParameterException, InvalidTokenException {
    ...
    request.addParameter(USER_TOKEN, userToken).addParameter(AREA, area);
  ...
  public Answer getAnswer(final Token userToken, final long areaId, final long questionId,
    ...
    request.addParameter(USER_TOKEN, userToken)
            .addParameter(AREA_ID, areaId)
            .addParameter(QUESTION_ID, questionId)
            .addParameter(ANSWER_ID, answerId);
